...
RegisterNetEvent('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		if PoliceBlip == 0 then
			...
			AddTextComponentSubstringPlayerName('Нападение на перевозку наличных')
		...
		if PoliceDist <= 4.5 then
			...
			if SilenceAlarm == 0 then
				hintToDisplay('Нажмите ~INPUT_DETONATE~, чтобы отключить сигнализацию')
				SilenceAlarm = 1
			end
...
RegisterNetEvent('qb-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
	if PlayerJob.name == 'police' then
		local store = 'Бронированный грузовик'
		PlaySound(-1, 'Lose_1st', 'GTAO_FM_Events_Soundset', 0, 0, 1)
		TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
			timeOut = 10000,
			alertTitle = 'Попытка ограбления бронированного грузовика',
			coords = {
				x = coords.x,
				y = coords.y,
				z = coords.z,
...
function MissionNotification()
	...
	TriggerServerEvent('qb-phone:server:sendNewMail', {
		sender = 'Босс',
		subject = 'Новая цель',
		message = 'Хочешь заработать деньжат? Отлично... раздобудь пушку и сделай всё как надо... высылаю тебе координаты.',
	})
end
	...
	CreateThread(function()
		...
		while not spawned do
			...
			if distance < 200 then
				...
				BeginTextCommandSetBlipName('STRING')
				AddTextComponentSubstringPlayerName('Фургон с наличными')
		...
		while MissionStart == 1 do
			...
			if dist <= 55.0 then
				...
				if warning == 0 then
					warning = 1
					stopAndBeAngry()
					QBCore.Functions.Notify('Сначала разберитесь с охраной, прежде чем закладывать бомбу.', 'error')
					AlertPolice()
				end
				...
				if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
					if BlowBackdoor == 0 then
						hintToDisplay('Нажмите [G], чтобы взорвать заднюю дверь и забрать деньги')
	...
	if IsVehicleStopped(transport) then
		if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				hideLastHint()
				QBCore.Functions.Notify('Заряд сработает через ' .. Config.TimeToBlow .. ' секунд.', 'error')
				...
				BlownUp = 1
				lootable = 1
				QBCore.Functions.Notify('Можно собирать наличные.', 'success')
				RemoveBlip(TruckBlip)
			else
				QBCore.Functions.Notify('Выйдите из воды', 'error')
			end
		else
			QBCore.Functions.Notify('Машина должна быть пустой, чтобы заложить заряд', 'error')
		end
	else
		QBCore.Functions.Notify('Нельзя грабить движущуюся машину.', 'error')
	end
end
...
CreateThread(function()
	while true do
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				sleep = 0
				hintToDisplay('Нажмите [E], чтобы забрать деньги')
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('Вы складываете наличные в сумку', 'success')
	...
	while GetGameTimer() - _time < 20000 do
		...
		hintToDisplay('Удерживайте [G], чтобы выпрыгнуть')
		Wait(0)
	end
