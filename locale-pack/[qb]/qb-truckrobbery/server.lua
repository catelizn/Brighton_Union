...
RegisterServerEvent('AttackTransport:akceptujto', function()
	...
	if ActiveMission == 0 then
		if accountMoney < Config.ActivationCost then
			TriggerClientEvent('QBCore:Notify', _source, 'Для принятия задания нужно ' .. Config.Currency .. '' .. Config.ActivationCost .. ' на банковском счёте')
		else
			...
			if copsOnDuty >= Config.ActivePolice then
				...
				TriggerClientEvent('QBCore:Notify', _source, 'Для задания нужно минимум ' .. Config.ActivePolice .. ' полицейских на дежурстве.')
			end
		end
	else
		TriggerClientEvent('QBCore:Notify', _source, 'Кто-то уже выполняет это задание')
	end
end)
...
RegisterServerEvent('AttackTransport:graczZrobilnapad', function()
	...
	TriggerClientEvent('QBCore:Notify', _source, 'Вы забрали ' .. bags .. ' сумок с наличными из фургона')
