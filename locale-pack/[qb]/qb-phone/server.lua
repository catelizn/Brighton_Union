...
-- Реальное время (МСК) для телефона: на клиенте os.* недоступен
QBCore.Functions.CreateCallback('qb-phone:server:GetMskTime', function(_, cb)
    local tzOffset = os.time() - os.time(os.date('!*t', os.time()))
    cb(os.time() - (tzOffset - 3 * 3600))
end)
...
RegisterNetEvent('qb-phone:server:InstallApplication', function(ApplicationData)
    ...
    Player.SetMetaData('phonedata', Player.PlayerData.metadata['phonedata'])
    ...
    if args[1] then
        if args[1] == 'trucker' then
            if args[2] then
                ...
                Player.SetMetaData('jobrep', newrep)
            end
        end
    end
end, 'god')
