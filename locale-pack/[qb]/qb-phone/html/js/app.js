    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                } else if (PressedApplication == "rental") {
                    $.post('https://qb-phone/GetRentalData', JSON.stringify({}), function(data){
                        SetupRentalInfo(data);
                    });
                } else if (PressedApplication == "parking") {
                    $.post('https://qb-phone/GetParkingData', JSON.stringify({}), function(data){
                        SetupParkingInfo(data);
                    });
                } else if (PressedApplication == "taxi") {
                    $.post('https://qb-phone/GetTaxiData', JSON.stringify({}), function(data){
                        SetupTaxiInfo(data);
                    });
                } else if (PressedApplication == "documents") {
                    QB.Phone.Functions.Close();
                    $.post('https://qb-phone/OpenDocuments', JSON.stringify({}));
                }
