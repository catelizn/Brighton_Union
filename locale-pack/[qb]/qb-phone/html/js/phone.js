...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Хм, не должно было так получиться...'
        ...
        $("#total-contacts").text(TotalContacts+ " контактов");
    } else {
        $("#total-contacts").text("0 контактов");
    }
};
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        if (SelectedSuggestion !== null) {
            ...
            $(".amount-of-suggested-contacts").html(amount + " контактов");
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Добавить контакт", "Заполните все поля!");
    }
});
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        $(".amount-of-suggested-contacts").html(AmountOfSuggestions + " контактов");
        ...
        $(".amount-of-suggested-contacts").html("0 контактов");
    }
}
