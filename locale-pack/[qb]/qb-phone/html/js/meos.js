...
SetupMeosHome = function() {
    $("#meos-app-name").html("Добро пожаловать " + QB.Phone.Data.PlayerData.charinfo.firstname + " " + QB.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Имя: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Дата рождения: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Номер телефона: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Национальность: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Пол: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Квартира: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">В розыске: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Водительские права: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Имя: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+PreviousPersonData.citizenid+'</div>';
...
$(document).on('click', '.person-search-result-house', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Владелец: '+ClickedHouseData.charinfo.firstname+' '+ClickedHouseData.charinfo.lastname+'</div><div class="person-search-result-bsn">Дом: '+ClickedHouseData.label+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Адрес: '+ClickedHouseData.label+' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="'+ClickedHouseId+'"></i></div> <div class="person-search-result-number">Класс: '+ClickedHouseData.tier+'</div> <div class="person-search-result-nationality">Гараж: ' + GarageLabel + '</div>';
    ...
    if (OpenedHouse === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Владелец: '+PreviousPersonData.charinfo.firstname+' '+PreviousPersonData.charinfo.lastname+'</div> <div class="person-search-result-bsn">Дом: '+PreviousPersonData.label+'</div>';
        ...
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, person) {
                    var PersonElement = '<div class="person-search-result" id="person-'+i+'"><div class="person-search-result-name">Имя: '+person.firstname+' '+person.lastname+'</div> <div class="person-search-result-bsn">CSN: '+person.citizenid+'</div> </div>';
        ...
        $.post('https://qb-phone/FetchPlayerHouses', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, house) {
                    var PersonElement = '<div class="person-search-result-house" id="personhouse-'+i+'"><div class="person-search-result-name">Владелец: '+house.charinfo.firstname+' '+house.charinfo.lastname+'</div> <div class="person-search-result-bsn">Дом: '+house.label+'</div></div>';
                    $(".person-search-results").append(PersonElement);
