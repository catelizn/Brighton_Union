local QBCore = exports['qb-core']:GetCoreObject({ 'Functions', 'Commands', 'Shared' })
local Accounts = {}
local Statements = {}
...
-- Лицевой номер счёта: создаётся один раз, по нему игроки переводят друг другу деньги
local function getOrCreateAccountNumber(citizenid)
    local result = MySQL.query.await('SELECT account_number FROM bu_bank_accounts WHERE citizenid = ? LIMIT 1', { citizenid })
    if result and result[1] then return result[1].account_number end

    local number = tostring(math.random(1000000000, 9999999999))
    MySQL.insert('INSERT IGNORE INTO bu_bank_accounts (citizenid, account_number) VALUES (?, ?)', { citizenid, number })

    local retry = MySQL.query.await('SELECT account_number FROM bu_bank_accounts WHERE citizenid = ? LIMIT 1', { citizenid })
    if retry and retry[1] then return retry[1].account_number end
    return citizenid
end
...
QBCore.Functions.CreateCallback('qb-banking:server:openBank', function(source, cb)
    ...
    cb(accounts, statements, Player.PlayerData, getOrCreateAccountNumber(citizenid))
end)

-- Банки работают с 09:00 до 21:00 по МСК, банкоматы — круглосуточно
QBCore.Functions.CreateCallback('qb-banking:server:isBankOpen', function(source, cb)
    local msk = os.date('*t', os.time() + 3 * 3600)
    cb(msk.hour >= 9 and msk.hour < 21)
end)
...
QBCore.Functions.CreateCallback('qb-banking:server:externalTransfer', function(source, cb, data)
    ...
    local targetCid = toAccountName
    local directPlayer = exports['qb-core']:GetPlayerByCitizenId(tostring(toAccountName))
    if not directPlayer and tostring(toAccountName):len() >= 10 then
        local lookup = MySQL.query.await('SELECT citizenid FROM bu_bank_accounts WHERE account_number = ? LIMIT 1', { tostring(toAccountName) })
        if lookup and lookup[1] then targetCid = lookup[1].citizenid end
    end
    local toPlayer = exports['qb-core']:GetPlayerByCitizenId(targetCid)
...
CreateThread(function()
    MySQL.query.await([[CREATE TABLE IF NOT EXISTS `bu_bank_accounts` (
        ...
        UNIQUE KEY `uq_number` (`account_number`)
...
QBCore.Commands.Add('givecash', 'Give Cash', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Amount' } }, true, function(source, args)
    ...
    Player.RemoveMoney('cash', amount, 'cash transfer')
    target.AddMoney('cash', amount, 'cash transfer')
