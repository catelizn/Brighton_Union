...
-- =====================================================================
-- Создание персонажа: предпросмотр родителей и наборы одежды
-- =====================================================================
...
local function enableCam()
    ...
    -- Камера смотрит на игрока с улицы — естественное освещение
...
local function parentNamesList(names)
    local list = {}
...
RegisterNetEvent('qb-clothes:client:CreateFirstCharacter', function()
    QBCore.Functions.GetPlayerData(function(pData)
        ...
        -- Создание персонажа: светлый день — интерьер хорошо освещён
        TriggerEvent('qb-weathersync:client:DisableSync')
        NetworkOverrideClockTime(12, 0, 0)
        SetWeatherTypeNow('EXTRASUNNY')

        -- Камера из конфига — та же точка, откуда видно выбор персонажа
        customCamLocation = Config.CreationCam

        -- Запоминаем точку: после смены модели педа надо вернуть на место
        local spot = GetEntityCoords(PlayerPedId())

        -- На улице поправляем высоту по земле, иначе персонаж висит в воздухе.
        -- В интерьерах луч врёт (перекрытия), поэтому принимаем только разницу
        -- меньше метра и оставляем точку как есть.
        local found, ground = GetGroundZFor_3dCoord(spot.x, spot.y, spot.z + 1.0, true)
        if found and math.abs(ground - spot.z) < 1.0 then
            spot = vector3(spot.x, spot.y, ground)
        end

        -- Игрок выбирает только внешность и три набора одежды:
        -- остальную одежду покупают в магазине за заработанные деньги
        openMenu({
            { menu = 'character', label = 'Черты лица', selected = true },
            { menu = 'hair',      label = 'Внешность',   selected = false },
            { menu = 'presets',   label = 'Гардероб', presets = Config.CreationOutfits[gender], selected = false }
        })

        SendNUIMessage({ action = 'creationMode', enabled = true })

        ChangeToSkinNoUpdate(skin)
        SendNUIMessage({
            action = 'ResetValues',
        })
        -- Имена родителей для сетки наследственности
        SendNUIMessage({
            action = 'createParents',
            mothers = parentNamesList(Config.MotherNames),
            fathers = parentNamesList(Config.FatherNames)
        })

        -- Держим педа на месте: смена модели и физика интерьера (MLO) иногда
        -- роняют персонажа под пол, а лучи GetGroundZFor_3dCoord в интерьерах врут
        Citizen.CreateThread(function()
            for _ = 1, 30 do
                local ped = PlayerPedId()
...
RegisterNetEvent('QBCore:Client:OnPlayerUpdated', function(key, val)
    ...
    elseif key == 'all' then
        PlayerData.job = val.job
        PlayerData.gang = val.gang
    end
end)
...
-- Набор одежды из трёх вариантов на экране создания персонажа
RegisterNUICallback('selectPreset', function(data, cb)
    local gender = 'male'
    if QBCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = 'female' end

    local presets = Config.CreationOutfits[gender]
    local index = tonumber(data.index) or 0
    -- Интерфейс нумерует наборы с нуля, в Lua таблица начинается с единицы
    local preset = presets and (presets[index] or presets[index + 1])
    if preset then
        for category, value in pairs(preset.items) do
            ChangeVariation({ clothingType = category, articleNumber = value.item, type = 'item' })
            ChangeVariation({ clothingType = category, articleNumber = value.texture, type = 'texture' })
        end
    end
    cb('ok')
end)
        ...
        CreateThread(function()
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not QBCore.Shared.QBJobsStatus then
                    ...
                    break
                end            --this break is important if QBJobsStatus is true then the reste of the code in this loop does not need to run.
...
-- Brighton Union: продавцы и подсказки [E] в магазинах одежды, барбершопах
-- и у хирургов. В qb-target зонах NPC не было видно вообще.
CreateThread(function()
    ...
    local pedModels = {
        clothing = 'a_f_y_business_04',
        barber = 'a_m_y_hipster_01',
        surgeon = 's_m_m_doctor_01'
    }
    local pedLabels = {
        clothing = 'Продавец одежды',
        barber = 'Барбер',
        surgeon = 'Пластический хирург'
    }

    for _, v in pairs(Config.Stores) do
        local shopType = v.shopType or 'clothing'
        ...
        exports['bu-interact']:spawnPed(pedModels[shopType] or 'a_f_y_business_04', vector4(coords.x, coords.y, coords.z, 0.0), {
            label = pedLabels[shopType] or 'Продавец',
            hint = 'Открыть меню',
            scenario = 'WORLD_HUMAN_STAND_IMPATIENT',
            action = open,
            plate = { text = pedLabels[shopType] or 'Продавец', range = 15.0 }
        })
    end
end)