    ...
    window.addEventListener('message', function(event) {
        ...
        switch (event.data.action) {
            ...
            case "ResetValues":
                ...
                break;
            case "createParents":
                QBClothing.parentNames.mothers = event.data.mothers || [];
                QBClothing.parentNames.fathers = event.data.fathers || [];
                QBClothing.RenderParents();
                QBClothing.RefreshParents();
                break;
            case "creationMode":
                $('body').toggleClass('creating', event.data.enabled !== false);
                break;
        }
    })
});
...
QBClothing.parentNames = { mothers: [], fathers: [] };

// Сетка родителей: фото + имя, клик сразу меняет генетику
QBClothing.RenderParents = function() {
    $.each(['mothers', 'fathers'], function(_, key) {
        const strip = $('#' + key + '-strip');
        const parentType = key === 'mothers' ? 'face' : 'face2';

        strip.html('');
        $.each(QBClothing.parentNames[key], function(_, parent) {
            const photo = 'images/parents/' + String(parent.name).toLowerCase() + '.webp';
            const card = $('<div class="parents-card" data-parent-type="' + parentType + '" data-parent-id="' + parent.id + '"><img class="parents-photo" src="' + photo + '" alt=""><span>' + parent.name + '</span><small>#' + parent.id + '</small></div>');
            strip.append(card);
        });
    });
};

QBClothing.RefreshParents = function() {
    $('.parents-card').each(function() {
        const card = $(this);
        const input = $('.clothing-menu-container').find('[data-type="' + card.data('parentType') + '"]').find('input[data-type="item"]');
        const value = parseInt($(input).val(), 10);
        card.toggleClass('selected', parseInt(card.data('parentId'), 10) === value);
    });
};

$(document).on('click', '.parents-card', function() {
    if (!canChange) return;
    const card = $(this);
    const parentType = card.data('parentType');
    const id = parseInt(card.data('parentId'), 10);

    $('.clothing-menu-container').find('[data-type="' + parentType + '"]').find('input[data-type="item"]').val(id);
    $.post('https://qb-clothing/updateSkin', JSON.stringify({
        clothingType: parentType,
        articleNumber: id,
        type: 'item',
    }));
    QBClothing.RefreshParents();
...
$(document).on('change input', '.item-number', function() {
    QBClothing.RefreshParents();
});

$(document).on('click', '.clothing-menu-preset-option-button', function(e) {
    e.preventDefault();
    var card = $(this).closest('.clothing-menu-option');
    var index = card.data('presetIndex');
    card.addClass('selected-preset').siblings('.clothing-preset').removeClass('selected-preset');
    $.post('https://qb-clothing/selectPreset', JSON.stringify({ index: index }));
    ...
    $(".clothing-menu-presets-container").css("display", "none").html("");
    ...
    $.each(data.menus, function(i, menu) {
        ...
        if (menu.menu == "presets") {
            $.each(menu.presets, function(index, preset) {
                var elem = '<div class="clothing-menu-option clothing-preset" data-preset="' + index + '"> <div class="clothing-menu-option-header"><p>' + preset.label + '</p></div> <div class="clothing-menu-preset-option-button"><p data-tkey="wear">Надеть</p></div> </div>'
                $(".clothing-menu-presets-container").append(elem)

                $("[data-preset='" + index + "']").data('presetIndex', index)
            });
        }
    });
...
QBClothing.Close = function() {
    ...
    $(".clothing-menu-roomOutfits-container").css("display", "none");
    $(".clothing-menu-myOutfits-container").css("display", "none");
    $(".clothing-menu-presets-container").css("display", "none");
    $(".clothing-menu-character-container").css("display", "none");
    $(".clothing-menu-clothing-container").css("display", "none");
    $(".clothing-menu-hair-container").css("display", "none");
    $(".clothing-menu-accessoires-container").css("display", "none");
