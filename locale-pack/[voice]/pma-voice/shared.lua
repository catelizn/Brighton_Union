...
if GetConvar('voice_useNativeAudio', 'false') == 'true' then
	...
	Cfg.voiceModes = {
		{ 1.5, "Шёпот" }, -- Whisper speech distance in gta distance units
		{ 3.0, "Обычный" },  -- Normal speech distance in gta distance units
		{ 6.0, "Крик" } -- Shout speech distance in gta distance units
	}
else
	Cfg.voiceModes = {
		{ 3.0,  "Шёпот" }, -- Whisper speech distance in gta distance units
		{ 7.0,  "Обычный" },  -- Normal speech distance in gta distance units
		{ 15.0, "Крик" } -- Shout speech distance in gta distance units
	}
end
