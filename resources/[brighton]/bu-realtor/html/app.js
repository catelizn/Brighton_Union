...
function fetchListings() { fetch('https://bu-realtor/getListings', { method: 'POST', body: '{}' }); }
function fetchMine() { fetch('https://bu-realtor/getMine', { method: 'POST', body: '{}' }); }
...
function renderListings(list) {
    ...
    (list || []).forEach((l) => {
        ...
        div.className = 'item';
        div.innerHTML = '<div><div class="i-label">' + l.label + '</div><div class="i-sub">' + l.owner + '</div></div>' +
            '<span class="i-price">$' + l.price + '</span><button class="act" data-id="' + l.id + '">Купить</button>';
        div.querySelector('.act').addEventListener('click', () => {
            fetch('https://bu-realtor/buy', { method: 'POST', body: JSON.stringify({ id: l.id }) });
...
function renderMine(list) {
    ...
    (list || []).forEach((m) => {
        const div = document.createElement('div');
        div.className = 'item';
        div.innerHTML = '<div class="i-label">' + m.label + '</div>' +
            '<input class="price-input" placeholder="Цена" />' +
            '<button class="act small" data-key="' + m.key + '">Продать</button>';
        div.querySelector('.act').addEventListener('click', () => {
            const price = div.querySelector('.price-input').value;
            fetch('https://bu-realtor/list', { method: 'POST', body: JSON.stringify({ key: m.key, price: price }) });
            setTimeout(fetchMine, 600);
        });
        el.appendChild(div);
    });
    if (!list || list.length === 0) el.innerHTML = '<div class="item">У тебя нет бизнесов</div>';
}
...
document.getElementById('close-btn').addEventListener('click', () => fetch('https://bu-realtor/close', { method: 'POST', body: '{}' }));
